...
describe('computeWindowTitle', () => {
  ...
  it('should use default Otter title when CLI_TITLE is not set', () => {
    const result = computeWindowTitle('my-project');
    expect(result).toBe('Otter - my-project');
  });
  ...
  it('should handle folder names with special characters', () => {
    ...
    expect(result).toBe('Otter - project-name_v1.0');
  });
});
